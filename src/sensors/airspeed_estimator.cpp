...
computeSaintVenantAirspeed(double static_pressure_pa,
  ...
  if (!std::isfinite(static_pressure_pa) ||
      ...
      !std::isfinite(pitot_coefficient) || pitot_coefficient <= 0.0)
  ...
  const double pressure_ratio =
      1.0 + differential_pressure_pa / static_pressure_pa;
  if (!std::isfinite(pressure_ratio) || pressure_ratio < 1.0)
    return {0.0, AirspeedModelError::numeric_error, false};
  const double exponent =
      (kHeatCapacityRatioAir - 1.0) / kHeatCapacityRatioAir;
  const double expansion = std::pow(pressure_ratio, exponent) - 1.0;
  ...
  if (!std::isfinite(radicand) || radicand < 0.0)
    return {0.0, AirspeedModelError::numeric_error, false};
  const double airspeed = pitot_coefficient * std::sqrt(radicand);

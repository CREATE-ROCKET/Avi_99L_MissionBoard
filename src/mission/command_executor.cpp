...
bool allZero(const GenericCommandRequest &request, std::size_t first) {
  return std::all_of(request.arguments.begin() + first,
...
bool argumentsValid(const GenericCommandRequest &request, CommandCode code) {
  ...
  case CommandCode::para_move_relative: {
    ...
    const int32_t tenths = static_cast<int16_t>(raw);
    return std::abs(tenths) < 1800;
  }
  case CommandCode::set_para_open:
  case CommandCode::set_para_close:
...
CommandDecision CommandExecutor::begin(const GenericCommandRequest &request,
  ...
  if (rejection != CommandReason::none) {
    const auto result =
        resultFor(request, CommandPhase::rejected, rejection);
...
EmergencyDecision
CommandExecutor::actuatorEmergency(uint8_t transaction_id,
                                   MissionState state) {
  EmergencyDecision decision{};
  const bool accepted = transaction_id != 0 &&
                        state == MissionState::command_receive;
  decision.result = {transaction_id,
                     static_cast<uint8_t>(CommandCode::actuator_emergency_result),
                     accepted ? CommandPhase::completed
...
CommandResult
CommandExecutor::liftoffEmergencyResult(uint8_t transaction_id,
                                        bool accepted) {
  ...
  const auto iterator =
      std::find_if(entries_.begin(), entries_.end(),
                   [transaction_id](const Entry &entry) {
                     return entry.valid &&
                            entry.request.transaction_id == transaction_id;
                   });

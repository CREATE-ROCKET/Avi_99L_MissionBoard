...
from verify_characterization import ValidationError, build_golden_fixture, verify_file
...
def _parse_uart(values: list[str]) -> dict[str, Path]:
    ...
    for value in values:
        ...
        if not separator or stage not in STAGES or stage in result:
            raise PackageError(f"--uart must be a unique STAGE=PATH entry, got {value!r}")
        result[stage] = Path(path)
    if set(result) != set(STAGES):
        raise PackageError("one --uart STAGE=PATH entry is required for every stage")
    return result


def _manifest_entry(path: Path, root: Path, **metadata: object) -> dict[str, object]:
    return {"path": path.relative_to(root).as_posix(), "sha256": _sha256(path), **metadata}
...
def _csv_rows(captures: list[tuple[Path, dict[str, object]]]) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    for package_path, capture in captures:
        ...
        for epoch in capture["epochs"]:
            ...
            epoch_row = {key: value for key, value in epoch.items() if key != "raw_samples"}
            ...
            current = (epoch["profile_phase_value"], epoch["episode_id"], epoch["abort_reason_value"])
            if current != previous:
                events.append(
                    {
                        "source_path": package_path.as_posix(),
...
def package(
    ...
    if not operator_label or any(ord(character) < 0x20 for character in operator_label):
    ...
    for source in captures:
        ...
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*\.bin", source.name):
            raise PackageError(f"capture filename is not portable: {source.name!r}")
        if not capture["integrity"]["strict_pass"]:
            raise PackageError(f"capture did not pass strict validation: {source}")
        ...
        if campaign_contract is None:
            ...
            raise PackageError("captures contain multiple firmware or physical contracts")
        stage = str(metadata["stage"])
        rate = int(metadata["encoder_rate_hz"])
        profile = str(metadata["run_kind"])
        coverage.setdefault((stage, rate), set()).add((profile, str(footer["completion"])))
        decoded.append((Path(stage) / f"{rate}Hz" / source.name, capture, source))

    assert session_id is not None
    if "session_id" in conditions and conditions["session_id"] != session_id:
        raise PackageError("conditions.json session_id does not match capture headers")
    if "profile_seed" in conditions and (
        ...
        raise PackageError("conditions.json profile_seed does not match capture headers")
    conditions = {
        **conditions,
    ...
    for stage in STAGES:
        ...
        for rate in RATES[1:]:
            ...
            if not any(
                ...
                for completion in ("normal", "unsupported")
            ):
                raise PackageError(f"{stage} {rate} Hz lacks rate-check evidence")
            if ("full", "normal") in outcomes and ("rate-check", "normal") not in outcomes:
                raise PackageError(f"{stage} {rate} Hz full capture lacks a successful rate-check")

    output_directory.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=".99l-package-", dir=output_directory.parent) as temporary:
        ...
        for relative, capture, source in decoded:
            ...
            if copied_capture["integrity"]["sha256"] != capture["integrity"]["sha256"]:
                raise PackageError(f"capture changed while it was being packaged: {source}")
            ...
            manifest_entries.append(
                _manifest_entry(
                    ...
                    start_timestamp_us=epochs[0]["epoch_start_timestamp_us"] if epochs else metadata["epoch_zero_timestamp_us"],
                    end_timestamp_us=epochs[-1]["epoch_end_timestamp_us"] if epochs else metadata["epoch_zero_timestamp_us"],
                    ...
                    unsupported_reason=capture["footer"]["unsupported_reason"],
                    unsupported_reason_value=capture["footer"]["unsupported_reason_value"],
                    rate_check_valid_epochs=capture["footer"]["rate_check_valid_epochs"],
                    rate_check_total_epochs=capture["footer"]["rate_check_total_epochs"],
                    first_error=capture["footer"]["first_error"],
                    shutdown_step_mask=capture["footer"]["shutdown_step_mask"],
                )
            )
        ...
        for stage, source in uart_paths.items():
            ...
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(source, destination)
            manifest_entries.append(_manifest_entry(destination, root, stage=stage))

        conditions_destination = root / "conditions.json"
        conditions_destination.write_text(_json_text(conditions), encoding="utf-8")
        manifest_entries.append(_manifest_entry(conditions_destination, root))
        ...
        integrity_destination.write_text(_json_text(aggregate_integrity), encoding="utf-8")
        manifest_entries.append(_manifest_entry(integrity_destination, root))

        if include_csv:
            csv_directory = root / "optional_csv"
            csv_directory.mkdir()
            epoch_rows, sample_rows, event_rows = _csv_rows(packaged_captures)
            for name, rows in (("epochs.csv", epoch_rows), ("raw_samples.csv", sample_rows), ("events.csv", event_rows)):
                destination = csv_directory / name
                _write_csv(destination, rows)
                manifest_entries.append(_manifest_entry(destination, root))
        ...
        manifest_destination.write_text(_json_text(manifest), encoding="utf-8")

        sum_paths = [path for path in root.rglob("*") if path.is_file() and path.name != "SHA256SUMS"]
        sums = "".join(f"{_sha256(path)}  {path.relative_to(root).as_posix()}\n" for path in sorted(sum_paths))
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        for stage_value, stage in enumerate(STAGES, start=1):
            for rate in RATES:
                run_kind = 2 if rate == 1000 else 1
                path = inputs / f"{stage}_{rate}_{run_kind}.bin"
                path.write_bytes(
                    build_golden_fixture(
                        stage_value=stage_value,
                        encoder_rate_hz=rate,
                        run_kind=run_kind,
                        sequence_base=stage_value * 100_000 + rate,
                    )
                )
        conditions = root / "conditions.json"
        conditions.write_text('{"fixture":"self-test"}\n', encoding="utf-8")
        uart: dict[str, Path] = {}
        for stage in STAGES:
        ...
        assert all("source" not in capture for capture in integrity["captures"])
        assert (output / "optional_csv" / "raw_samples.csv").is_file()
        assert len(list(output.rglob("*.bin"))) == len(STAGES) * len(RATES)
        ...
        bad_inputs.mkdir()
        (bad_inputs / "capture\ninjected.bin").write_bytes(build_golden_fixture())
        try:
            package(bad_inputs, root / "bad-package", conditions, "self-test", uart, False)
        ...
        for name, malformed in (
            ...
            try:
                ...
                raise AssertionError(f"non-strict JSON was accepted: {name}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--uart", action="append", default=[], metavar="STAGE=PATH")
    ...
    try:
        if args.self_test:
            ...
            return 0
        if args.input is None or args.output is None or args.conditions is None or args.operator_label is None:
            raise PackageError("input, output, --conditions, and --operator-label are required")
        result = package(args.input, args.output, args.conditions, args.operator_label, _parse_uart(args.uart), args.csv)
